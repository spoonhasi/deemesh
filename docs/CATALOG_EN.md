...
**Writing follows the machine's parameter-write enable state.** If writing is not enabled, the machine rejects it and you get status `-22` (machine state). Setting this value incorrectly can stop an axis short of where it needs to travel, or loosen the protection, so on a real machine read the current value before changing it.
...
**No `unit` is attached, because this is a distance.** Millimetres or inches is a machine setting (handled like `axisSoftLimitPositive`). Writing follows the machine's parameter-write enable state; if writing is blocked the status is `-22` (machine state).
...
**No `unit` is attached, because this is a distance.** Millimetres or inches is a machine setting (handled like `axisSoftLimitNegative`). Writing follows the machine's parameter-write enable state; if writing is blocked the status is `-22` (machine state).
...
**Fanuc**: diagnosis `4902`. **Siemens**: there is no spindle-area node for this, so the value of that spindle's **machine axis** is read (`vaPower`). On the rare machine where the control does not map the spindle to a machine axis, the address answers status `-20`. ⚠️ **We have not yet seen this produce a value on Siemens** (it stayed `0` on our bench while a spindle turned at 1425 rpm; see `axisPower` for the detail).

**There is no address for total machine power.** Fanuc has a value the control measures itself, but Siemens has none, so we would have to add up the axes and spindles: the same address would then be a measured value on one control and a sum of ours on the other. **Even on Fanuc the total does not equal the sum of the parts**, because each value is sampled at a different instant. If you need a total, add the axis and spindle values yourself, and bear in mind it may differ from what the control measures.
...
**Siemens only** (`actToolIdent`). The other two controls answer status `-20`, and always will.
...
**It can differ from the tool number.** They match on many machines by coincidence, but they are separate values (measured: changing the tool number to `7` left the offset number at `5`).
...
The Mitsubishi panel's tool management table carries two offset columns (`X5` / `Y5`), and **one write changes both** (measured). That screen does not refresh on its own, so to check it you have to leave the screen and come back.
...
**Siemens only** (`toolMyMag`); the other two controls answer status `-20`. Whether those controls have the same concept has not been looked into yet.
...
**Siemens only** (`toolMyPlace`); the other two controls answer status `-20`.
